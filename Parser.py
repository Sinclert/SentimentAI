...
################# TRAIN TEST #################
# Arguments: "Train" <Classifier> <L1 file> <L2 file> <Words prop> <Bigrams prop> <Output>
...
if (len(sys.argv) == 8) and (sys.argv[1].lower() == "train"):
    ...
    if sys.argv[2].lower() in classifiers:
        ...
        try:
            ...
            print("ERROR: The specified proportions must be integers")
            exit()
...
# Arguments: "Stream" <Classifier 1> <Classifier 2> <Buffer size> <Query> <Language> <Coordinates>
    ...
    listener = TwitterListener(classifier1, classifier2, buffer_size, shared_dict)
...
# In case none of the possible options is selected: error
    ...
    print("Mode 1: 'Train' <Classifier> <L1 file> <L2 file> <Words prop> <Bigrams prop> <Output>")
